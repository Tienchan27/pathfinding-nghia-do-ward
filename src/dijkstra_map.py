...
def graph_stats():
    all_nodes = set()
    all_edges = set()  # dùng set để tránh đếm cạnh trùng (vì đồ thị vô hướng)

    # Giả sử helper.getAllNodes() trả về list tất cả node OSMId
    nodes = help.getAllNodes()  

    for node in nodes:
        all_nodes.add(node)
        for neighbor, _length in help.getAdjacentNodes(node):
            # lưu cạnh dạng tuple sắp xếp để tránh trùng
